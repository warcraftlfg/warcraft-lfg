...
module.exports.add = function(query,criteria){
    ...
    var paramQueryArray =  params.parseQueryParam(query.sort,1);

    if(paramQueryArray.length > 0){
        var sort = paramQueryArray[0][0];
        if (sort === "ranking") {
            criteria["wowProgress"] = {$exists: true};
        }
    }

    if(paramLastArray.length > 0 && paramQueryArray.length > 0){
